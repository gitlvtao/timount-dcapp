<?php /*a:1:{s:46:"D:\DCAPP\application\user\view\test\index.html";i:1542706660;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <!-- 下面是定义标签库 -->
    ...
    <div class="layui-card">
                ...
                <div class="layui-form-item">
                    <div class="layui-inline">
                        ...
                        <div class="layui-input-inline">
                            <input type="text" name="mobile" placeholder="模糊搜索" autocomplete="off" class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline">
        ...
        <div class="layui-card-body">
            <div style="padding-bottom: 10px;">
                <!--<button class="layui-btn layui-btn-sm" id="btn-add">新增广告位置</button>-->
            </div>
...
<script>
    ...
    layui.config({
        ...
        tableIns = table.render({
            ...
            url: '<?php echo url("user/test/index"); ?>',
            method: 'post',
            page: true,
            loading: true,
            limit: 20,
            ...
            cols: [[
                {type: 'numbers'},
                {field: 'user_mobile', minWidth: 140, title: '用户手机号'},
                {field: 'test_money', minWidth: 180, title: '贷款额度'},
                {field: 'test_term', minWidth: 180, title: '贷款期限'},
                {field: 'test_use', minWidth: 180, title: '贷款期限'},
                {field: 'test_follow', minWidth: 180, title: '关注项'},
                {field: 'test_create_time', width: 130, align: 'center', title: '创建日期', templet: function(d) {
                    return new Date(d.test_create_time*1000).Format("yyyy-MM-dd");
                }},
        ...
        $('#btn-add').click(function () {
            layer.open({
                type: 2,
                title: '新增角色',
                content: '<?php echo url("ads/positions/updateadd"); ?>',
        ...
        table.on('tool(layui-datatables)', function(obj){
            ...
            if (layEvent == "edit") {
                layer.open({
                    type: 2,
                    title: '修改角色信息',
                    content: '<?php echo url("ads/positions/updateadd"); ?>?pid='+data.positions_id,
                    area: layerArea2(), //计算页面大小
                });
            } else if (layEvent == "del") {
                layer.confirm('你确定要删除该角色?', {icon: 3, title:'提示'}, function(index){
                    layer.load(1);
                    $.post("<?php echo url('ads/positions/delete'); ?>",{pid:data.positions_id}, function(result){
                        layer.close(index);
                        layer.closeAll('loading');
                        if (result.code == 0) {
                            tableIns.reload();
