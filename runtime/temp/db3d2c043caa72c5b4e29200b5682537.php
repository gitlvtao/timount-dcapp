<?php /*a:1:{s:52:"D:\DCAPP\application\admin\view\adminuser\index.html";i:1540188023;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <!-- 下面是定义标签库 -->
        ...
        table.on('tool(layui-datatables)', function(obj){
            ...
            if (layEvent == "forbid") { // 禁用
                layer.confirm('你确定要禁用该账号?', {icon: 3, title:'提示'}, function(index){
                    layer.load(1);
                    $.post("<?php echo url('status'); ?>",{uid:data.user_id, type: "forbid"}, function(result){
                        layer.close(index);
                        layer.closeAll('loading');
                        if (result.code == 0) {
                            tableIns.reload();
                ...
                layer.confirm('你确定要启用该账号?', {icon: 3, title:'提示'}, function(index){
                    layer.load(1);
                    $.post("<?php echo url('status'); ?>",{uid:data.user_id, type: "reuse"}, function(result){
                        layer.close(index);
                        layer.closeAll('loading');
                        if (result.code == 0) {
                            tableIns.reload();
                ...
                layer.confirm('你确定要删除该账号?', {icon: 3, title:'提示'}, function(index){
                    layer.load(1);
                    $.post("<?php echo url('status'); ?>",{uid:data.user_id, type: "del"}, function(result){
                        layer.close(index);
                        layer.closeAll('loading');
                        if (result.code == 0) {
                            tableIns.reload();
                ...
                layer.open({
                    type: 2,
                    title: '修改用户信息',
                    content: '<?php echo url("updateadd"); ?>?uid=' + data.user_id,
                    area: layerArea2(), //计算页面大小
                });
            } else if (layEvent == "resetPwd") {
                layer.prompt({title: '请输入新的登录密码', formType: 1}, function(newPwd, index){
                    layer.load(1);
                    $.post("<?php echo url('resetpwd'); ?>",{uid: data.user_id,newPwd:newPwd}, function(result){
                        layer.close(index);
                        layer.closeAll('loading');
                        if (result.code == 0) {
